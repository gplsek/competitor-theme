...
var ReMooz = new Class({
	...
	build: function() {
		...
		if(this.element.get('rel'))
		{
			var adlink=this.element.get('rel');
			this.body = new Element('a', {'href': 'javascript:void(window.open("'+adlink+'"))'}).inject(new Element('div', {'class': 'remooz-body'}).inject(this.box));
		}
		else
		{
